    ...
    else
    {
        for (int i = 0; i < paths_len; i++)
        {
            ...
            if (can_access == 0)
            {
                ...
                command.program_wout_path[0] = 0;
                strcat(command.program_wout_path, command.program_w_args[0]);
...
int runCommands(command commands[], int command_count)
{
    ...
    for (int i = 0; i < command_count; i++)
    {
        ...
        else if (rc == 0)
        {
            ...
            char path[4096];
            strcat(path, command.program_w_args[0]);
            command.program_w_args[0][0] = 0;
            // memset(command.program_w_args[0],0,strlen(command.program_w_args[0]));
            strcat(command.program_w_args[0], command.program_wout_path);
            // printf("==> %s\n", command.program_w_args[0]);
            command.program_w_args[0][strlen(command.program_w_args[0])] = '\0';


            execv(path, command.program_w_args);
    ...
    for (int i = 0; i < command_count; i++)
    {
        waitpid((pid_t)child_pids[i], &status, 0);
    }
    return 0;
}
